...
public class TaskTool {
    ...
    @Tool(
            ...
            description = "创建一个新的任务。仅当用户明确要求创建、安排或添加任务时使用。"
                    + "时间参数必须使用 ISO-8601 格式（例如 2026-09-08T19:00:00）。"
                    + "如果用户使用'明天'、'下周三'、'今晚'等相对或模糊的时间描述，"
                    + "你应当结合当前日期将其换算为明确的日期时间后再调用本工具，"
                    + "不要因为用户没有直接给出 ISO 格式就拒绝调用。"
                    + "只有在确实无法推断出具体时间时，才不调用本工具，转而向用户询问。"
